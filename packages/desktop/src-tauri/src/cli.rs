...
/// Returns "opencode" so it is resolved from the user's PATH.
...
pub fn spawn_command(
    ...
    let mut cmd = if cfg!(windows) {
        if is_wsl_enabled(app) {
            ...
            // On Windows (non-WSL), invoke opencode directly; the OS resolves it via PATH.
        ...
        // On Unix, invoke through the user's login shell so PATH is inherited.
...
pub fn serve(
    ...
    port: u32,
    password: &str,
) -> Result<(CommandChild, oneshot::Receiver<TerminatedPayload>), std::io::Error> {
