...
pub fn init(log_dir: &Path) -> WorkerGuard {
    ...
    let filename = format!("alfalfa-desktop_{timestamp}.log");
    ...
    let filter = EnvFilter::try_from_default_env().unwrap_or_else(|_| {
        if cfg!(debug_assertions) {
            EnvFilter::new("alfalfa_lib=debug,alfalfa_desktop=debug,sidecar=debug")
        } else {
            EnvFilter::new("alfalfa_lib=info,alfalfa_desktop=info,sidecar=info")
        }
    });
