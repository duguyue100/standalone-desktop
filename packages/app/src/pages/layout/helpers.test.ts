...
describe("layout deep links", () => {
  test("parses open-project deep links", () => {
    expect(parseDeepLink("alfalfa://open-project?directory=/tmp/demo")).toBe("/tmp/demo")
  })

  test("ignores non-project deep links", () => {
    expect(parseDeepLink("alfalfa://other?directory=/tmp/demo")).toBeUndefined()
    expect(parseDeepLink("https://example.com")).toBeUndefined()
  })

  test("ignores malformed deep links safely", () => {
    expect(() => parseDeepLink("alfalfa://open-project/%E0%A4%A%")).not.toThrow()
    expect(parseDeepLink("alfalfa://open-project/%E0%A4%A%")).toBeUndefined()
  })
  ...
  test("ignores open-project deep links without directory", () => {
    expect(parseDeepLink("alfalfa://open-project")).toBeUndefined()
    expect(parseDeepLink("alfalfa://open-project?directory=")).toBeUndefined()
  })

  test("collects only valid open-project directories", () => {
    const result = collectOpenProjectDeepLinks([
      "alfalfa://open-project?directory=/a",
      "alfalfa://other?directory=/b",
      "alfalfa://open-project?directory=/c",
    ])
    expect(result).toEqual(["/a", "/c"])
  })

  test("parses new-session deep links with optional prompt", () => {
    expect(parseNewSessionDeepLink("alfalfa://new-session?directory=/tmp/demo")).toEqual({ directory: "/tmp/demo" })
    expect(parseNewSessionDeepLink("alfalfa://new-session?directory=/tmp/demo&prompt=hello%20world")).toEqual({
      directory: "/tmp/demo",
      prompt: "hello world",
    })
  })
  ...
  test("ignores new-session deep links without directory", () => {
    expect(parseNewSessionDeepLink("alfalfa://new-session")).toBeUndefined()
    expect(parseNewSessionDeepLink("alfalfa://new-session?directory=")).toBeUndefined()
  })

  test("collects only valid new-session deep links", () => {
    const result = collectNewSessionDeepLinks([
      "alfalfa://new-session?directory=/a",
      "alfalfa://open-project?directory=/b",
      "alfalfa://new-session?directory=/c&prompt=ship%20it",
    ])
  ...
  test("drains global deep links once", () => {
    const target = {
      __ALFALFA__: {
        deepLinks: ["alfalfa://open-project?directory=/a"],
      },
    } as unknown as Window & { __ALFALFA__?: { deepLinks?: string[] } }

    expect(drainPendingDeepLinks(target)).toEqual(["alfalfa://open-project?directory=/a"])
